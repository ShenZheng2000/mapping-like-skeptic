...
class VectorEvaluate(object):
    ...
    def __init__(self, dataset_cfg: Config, n_workers: int=N_WORKERS, tmp_gts_path=None) -> None:
        self.tmp_gts_path = tmp_gts_path
    ...
    def gts(self) -> Dict[str, Dict[int, List[NDArray]]]:
        ...
        elif self.tmp_gts_path is not None:
            tmp_file = self.tmp_gts_path
        else:
    ...
    def _evaluate_single(self, 
        ...
        return tp_fp_score_by_thr # {0.5: (M, 3), 1.0: (M, 3), 1.5: (M, 3)}
        
    def evaluate(self, 
                 result_path: str, 
                 metric: str='chamfer', 
                 logger: Optional[Logger]=None,
                 interested_tokens: Optional[List[str]] = None) -> Dict[str, float]:
        ...
        for token, gt in self.gts.items():
            if interested_tokens is not None and token not in interested_tokens:
                continue
        ...
        for label in self.id2cat.keys():
            ...
            for thr in self.thresholds:
                ...
                precisions = tp / np.maximum((tp + fp), eps) # instead of tp + fp, you could have written range(num_preds) + 1
    ...
    def periodic_evaluate(self,
                          result_path: str,
                          time_idx: int,
                          period: int,
                          metric: str='chamfer',
                          logger: Optional[Logger]=None) -> Dict[str, float]:
        # modify self.gts to do periodic evaluation
        #
        # first we need to create such a mapping:
        # {time_idx: [tokens], ... till time_idx == period - 1}
        # then we store the self.gts in a temp variable and create some copies which are composed of th
        # use pos_predictions.pkl to extract local_idx information for frames
        interested_tokens = list()
        pos_path = "/".join(result_path.split("/")[:-1])
        pos_path = pos_path + "/" + "pos_predictions.pkl"
        pos_results = mmcv.load(pos_path)
        for frame in pos_results:
            local_idx = frame["local_idx"]
            if local_idx % period == time_idx:
                interested_tokens.append(frame["meta"]["token"])

        to_ret = self.evaluate(result_path=result_path, metric=metric, logger=logger, interested_tokens=interested_tokens)
