...
def average_precision(recalls, precisions, mode='area'):
    ...
    if mode == 'area':
        ...
        for i in range(mpre.shape[1] - 1, 0, -1):
            # if decreasing the threshold increase the precision,
            # you can decrease it easily; so when calculating average precision,
            # if a precision value with higher threshold is smaller than the one with lower threshold,
            # you can assume for that precision value, you are using the precision of the one with lower threshold
            # but not the other way around,
            # you can act like you didn't use that threshold and directly get the result of the smaller one
    ...
    for thr in thresholds:
        ...
        for i in sort_inds:
            if matrix_min[i] <= thr:
                ...
                # if it is not already covered by a more confident prediction
                # cover it and count that prediction as tp
                # if it is already covered, count it as fp anyways
