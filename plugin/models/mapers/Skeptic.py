"""
    Skeptic main module, adapted from MapTracker
"""
import numpy as np
import torch
...
iou_dict = {"ped_crossing": list(), "divider": list(), "boundary": list()}


@MAPPERS.register_module()
class Skeptic(BaseMapper):
    ...
    def __init__(self,
        ...
        self.skip_vector_head = skip_vector_head  # bool
        self.freeze_bev = freeze_bev
        self.freeze_bev_iters = freeze_bev_iters
    ...
    def temporal_propagate(self, curr_bev_feats, img_metas, all_history_curr2prev, all_history_prev2curr, use_memory,
        ...
        '''
        Args:
            curr_bev_feats: torch.Tensor of shape [B, neck_input_channels, H, W]
        ...
        for b_i in range(bs):
            ...
            if track_query_info is not None:
                ...
                if get_trans_loss:
                    ...
                    pose_matrix_inv = torch.inverse(trans_matrix.cpu()).float()[:3].cuda()
                    ...
                    pred_backtrans = self.head.reg_branches[-1](
    ...
    def process_history_info(self, img_metas, history_img_metas):
        ...
        for b_i in range(bs):
            history_e2g_trans = torch.stack(
                [self.plane.new_tensor(prev[b_i]['ego2global_translation'], dtype=torch.float64) for prev in
                 history_img_metas], dim=0)
            history_e2g_rot = torch.stack(
                [self.plane.new_tensor(prev[b_i]['ego2global_rotation'], dtype=torch.float64) for prev in
                 history_img_metas], dim=0)
            ...
            history_g2e_matrix = torch.stack(
                [torch.eye(4, dtype=torch.float64, device=history_e2g_trans.device), ] * len(history_e2g_trans), dim=0)
            history_g2e_matrix[:, :3, :3] = torch.transpose(history_e2g_rot, 1, 2)
            history_g2e_matrix[:, :3, 3] = -torch.bmm(torch.transpose(history_e2g_rot, 1, 2),
                                                      history_e2g_trans[..., None]).squeeze(-1)
            ...
            # what are the coordinates of the current grid coordinates, in the eyes of the previous frame
            # so it maps the current grid coordinates (with index) with the coordinates of the previous frame (the values in that index)
            history_curr2prev_matrix = torch.bmm(history_g2e_matrix,
                                                 repeat(curr_e2g_matrix, 'n1 n2 -> r n1 n2', r=len(history_g2e_matrix)))
            history_prev2curr_matrix = torch.bmm(repeat(curr_g2e_matrix, 'n1 n2 -> r n1 n2', r=len(history_e2g_matrix)),
                                                 history_e2g_matrix)
    ...
    def forward_train(self, img, vectors, semantic_mask, points=None, img_metas=None, all_prev_data=None,
        ...
        history_bev_feats = []
        history_img_metas = []
        history_raw_bev_feats = []
        history_raw_bev_feats_conf = []
        ...
        for t in range(num_prev_frames):
            ...
            _bev_feats, merged_raw_bev_feats, merged_raw_conf_level, mlvl_feats = self.backbone(all_img_prev[t], all_img_metas_prev[t], t, history_bev_feats,
                                                   history_img_metas, all_history_coord, 
                                                   history_raw_bev_feats, history_raw_bev_feats_conf, all_hist_coords=[],
            ...
            if self.skip_vector_head or t == 0:
                self.temporal_propagate(bev_feats, all_img_metas_prev[t], all_history_curr2prev,
                                        all_history_prev2curr, _use_memory, track_query_info, timestep=t,
                                        get_trans_loss=False)
            else:
                trans_loss_dict = self.temporal_propagate(bev_feats, all_img_metas_prev[t], all_history_curr2prev,
                                                          ...
                                                          timestep=t, get_trans_loss=True)
            ...
            gts_semantic_curr = torch.flip(all_semantic_mask_prev[t + 1],
                                           [2, ]) if t != num_prev_frames - 1 else gt_semantic
            ...
            history_raw_bev_feats.append(merged_raw_bev_feats)
            history_raw_bev_feats_conf.append(merged_raw_conf_level)
            if len(history_bev_feats) > self.history_steps:
                history_bev_feats.pop(0)
                history_img_metas.pop(0)
                history_raw_bev_feats.pop(0)
                history_raw_bev_feats_conf.pop(0)
            ...
            if not self.skip_vector_head:
                ...
                track_query_info = self.prepare_track_queries_and_targets(gts_next, prev_inds_list,
                                                                          prev_gt_inds_list, prev_matched_reg_cost,
                                                                          prev_gt_list, outputs_prev, gt_cur2prev,
                                                                          gt_prev2cur,
                                                                          img_metas_prev, _use_memory, pos_th=pos_th,
                                                                          timestep=t)
            else:
                loss_dict_prev = {}
        ...
        # all_img_metas_prev[num_prev_frames - 1] this is the closest frame to the current one
        ###

        all_history_curr2prev, all_history_prev2curr, all_history_coord = self.process_history_info(img_metas,
                                                                                                    history_img_metas)

        _bev_feats, merged_raw_bev_feats, merged_raw_conf_level, mlvl_feats = self.backbone(img, img_metas, num_prev_frames, history_bev_feats, history_img_metas,
                                               all_history_coord,
                                               history_raw_bev_feats, history_raw_bev_feats_conf, all_hist_coords=[],
                                               points=None, img_backbone_gradient=img_backbone_gradient)
        # Neck for curr
        ...
        if self.skip_vector_head or num_prev_frames == 0:
            ...
            self.temporal_propagate(bev_feats, img_metas, all_history_curr2prev,
                                    all_history_prev2curr, _use_memory, track_query_info, timestep=num_prev_frames,
                                    get_trans_loss=False)
        else:
            trans_loss_dict = self.temporal_propagate(bev_feats, img_metas, all_history_curr2prev,
                                                      all_history_prev2curr, _use_memory, track_query_info,
                                                      timestep=num_prev_frames, get_trans_loss=True)
            all_trans_loss.append(trans_loss_dict)
    ...
    def forward_test(self, img, points=None, img_metas=None, seq_info=None, **kwargs):
        ...
        # scene_name, local_idx, seq_length = kwargs["seq_info"][0]
        ...
        if first_frame:
            ...
            self.history_raw_bev_feats_all = []
            self.history_raw_bev_feats_conf_all = []

        if self.use_memory:
            self.memory_bank.curr_t = local_idx

        # select according to the distance of cur frame to predetermined distances 1, 5, 10, 15
        ...
        # all_history_coord is how previous frames sees the current window,
        # the current window in the eyes of the previous frames
        all_history_curr2prev, all_history_prev2curr, all_history_coord = \
            self.process_history_info(img_metas, history_img_metas)
        
        _, _, every_single_coord = self.process_history_info(img_metas, self.history_img_metas_all)

        _bev_feats, merged_raw_bev_feats, merged_raw_conf_level, mlvl_feats = self.backbone(img, img_metas, local_idx, history_bev_feats, history_img_metas,
                                               all_history_coord, 
                                               self.history_raw_bev_feats_all, self.history_raw_bev_feats_conf_all, all_hist_coords=every_single_coord,
                                               points=points)  # bev_feats: 1, 256, 50, 100

        img_shape = [_bev_feats.shape[2:] for i in range(_bev_feats.shape[0])]
        # Neck
        bev_feats = self.neck(_bev_feats)

        gt_semantic = None  # for syntax purposes
        ...
        self.history_raw_bev_feats_all.append(merged_raw_bev_feats)
        self.history_raw_bev_feats_conf_all.append(merged_raw_conf_level)
        ...
        if len(self.history_bev_feats_all) > self.test_time_history_steps:
            ...
            self.history_raw_bev_feats_all.pop(0)
            self.history_raw_bev_feats_conf_all.pop(0)
        ...
        for b_i in range(len(results_list)):
            ...
            # replace the 0's at the places with (1, 2, or 3) where a class has high confidence
            # so we end up having a one channel img, where 0 means no pred for that pixel,
            # 1 means boundary, 2 means lane line, 3 means crosswalk (numbers might be incorrect for classes)
    ...
    def get_two_frame_matching(self, local2global_prev, local2global_curr, gts_prev, gts):
        ...
        all_gt_cur2prev, all_gt_prev2cur = multi_apply(self._compute_cur2prev, gt2local_curr, gt2local_prev,
                                                       local2gt_prev,
    ...
    def prepare_track_queries_and_targets(self, gts, prev_inds_list, prev_gt_inds_list, prev_matched_reg_cost,
                                          prev_gt_list, prev_out, gt_cur2prev, gt_prev2cur, metas_prev, use_memory,
                                          pos_th=0.4, timestep=None):
        ...
        for b_i, (target, prev_out_ind, prev_target_ind) in enumerate(zip(targets, prev_inds_list, prev_gt_inds_list)):
            ...
            if use_memory:
                ...
                self.memory_bank.update_memory(b_i, is_first_frame, prev_out_ind_final, prev_out, num_tracks,
                                               scene_seq_id, timestep)
    ...
    def _freeze_all(self):
        for param in self.parameters():  # Default is recurse=True
            param.requires_grad = False

    def _unfreeze_simple_cnn_predictor(self):
        for param in self.backbone.simple_cnn_predictor.parameters():
