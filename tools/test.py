...
def main():
    ...
    if args.work_dir is not None:
        ...
        if cfg.get('backbone_process_period', None) is not None:
            process_period = cfg.backbone_process_period
            cfg.work_dir = osp.join(cfg.work_dir, f"process_period_{process_period}")
    ...
    if args.result_path:
        process_period = cfg.get('backbone_process_period', None)
        """
        if process_period is not None and process_period > 1:
            dataset.evaluate_wrt_time(args.result_path, process_period)
            return
        """
        # outputs = args.result_path
        dataset._evaluate(args.result_path, kwargs=cfg.get("evaluation"))
        return
    ...
    if rank == 0:
        ...
        if args.format_only:
            # results, denormalize = True,
            # prefix = self.work_dir, save_semantic = save_semantic
            kwargs["prefix"] = cfg.data.test.work_dir
